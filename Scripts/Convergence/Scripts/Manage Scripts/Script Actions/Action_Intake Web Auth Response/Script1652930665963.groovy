...
selenium.type('id=scriptNameInput', 'Intake Web Auth Response')
...
selenium.type('id=scriptDescriptionInput', 'Intake Web Auth Response')
...
assertEquals('An indicator if the auth was successful. A value of Yes, Y, True, T, or 1 is interpreted as success. Any other value is unsuccess.', 
    selenium.getText('//div[contains(text(),"A value ")]'))

assertEquals('Failure Reason', selenium.getText('//label[@for="FailureReasonInput"]'))
...
assertEquals('If the auth was unsuccessful, this indicates the reason. This value will be passed to system level action \'loginFailure\' as a query parameter named \'reason\'.', 
    selenium.getText('//div[contains(text(),"If the auth was unsuccessful, this indicates the r")]'))

assertEquals('Roles', selenium.getText('//label[@for="RolesInput"]'))
...
assertEquals('If the auth was successful, a comma separated list of roles associated to the user. Note that no roles is not the same as an anonymous user.', 
    selenium.getText('//div[contains(text(),"If the auth was successful, a comma separated list")]'))

assertEquals('User Description', selenium.getText('//label[@for="UserDescriptionInput"]'))
...
assertEquals('If the auth was successful, an optional description of the user (usually a name).', selenium.getText('//div[contains(text(),"If the auth was successful, an optional descriptio")]'))
