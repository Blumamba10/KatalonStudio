...
WebUI.click(findTestObject('Convergence/Scripts/ManageFunctionsPage/Script Editor_Page/_Select an Action or Script_popup/Actions/a_AuthUser'))
...
assertEquals('Mode', selenium.getText('xpath=(.//*[normalize-space(text()) and normalize-space(.)=\'Description\'])[2]/following::label[1]'))

assertEquals('Password AuthToken AuthNTLM AuthSAML Auth', selenium.getText('id=ModeSelectTriggerInput'))
...
assertEquals('Auto Create LDAP Authenticated Users', selenium.getText('//label[@for="AutoCreateLDAPAuthenticatedUsersSelectInput"]'))

assertEquals('Default to server settingTurn off Auto Create', selenium.getText('id=AutoCreateLDAPAuthenticatedUsersSelectInput'))

selenium.click('id=nameInput')

selenium.typeKeys('id=nameInput', 'AuthUser')
...
WebUI.click(findTestObject('Convergence/Scripts/ManageFunctionsPage/Script Editor_Page/button_Save'))
...
WebUI.setText(findTestObject('Convergence/Scripts/ManageScriptsPage/input_Search Filter'), 
    'Auth User')


assertEquals('Auth User', selenium.getText('//a[@class="new-editor-open"]'))
